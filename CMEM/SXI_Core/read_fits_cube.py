...
class read_fits_cube():
    ...
    def __init__(self, filename='S05D05V400B0000-05rad.fits', filetype='PPMLR', xmin=None, xmax=None, ymin=None, ymax=None, zmin=None, zmax=None): 
        ...
        try:
            ...
            with pyfits.open(self.filename_fits) as hdul:
                ...
                #Get solar wind flux. 
                self.flux = abs(self.vx*1e5*self.density)
            ...
            raise SystemExit("Filename not found: {}".format(self.filename_fits))
    ...
    def get_subsolar_magnetopauses(self):
        ...
        #Ensure you only include values in the positive x axis. 
        self.xp = xp[xp>0]
        self.yp = yp[xp>0]
        self.zp = zp[xp>0]
        self.etad = etad[xp>0] 
        ...
        # Get max Ix second. 
        ix_index = np.where(self.etad == self.etad.max())
        self.maxIx = self.xp[ix_index][0]
        self.maxIx_eta = self.etad.max()
        
        # Get max dIx third. 
        # Get difference between etad values. 
        if self.filetype == 'OPENGGCM':
            dIx = -np.array([self.etad[i+1] - self.etad[i] for i in range(len(self.etad) - 1)])
        else:
            dIx = np.array([self.etad[i+1] - self.etad[i] for i in range(len(self.etad) - 1)])
            
        # Get centre point positions for radial direction. 
        xp_cent = self.xp + (self.xp[1]-self.xp[0])/2
        xp_cent = xp_cent[0:-1]

        dix_index = np.where(dIx == dIx.max())
        self.maxdIx = self.xp[dix_index][0]

        # Get f=0.25 
        dr = self.xp[ix_index] - self.xp[dix_index]
        self.f = self.xp[dix_index] + 0.25*dr[0] 
        
    def trapezium_rule_varied(self, pres, eta_LOS):
        '''This trapezium rule takes account of uneven spacing. From JXI. ''' 
        
        #return 0.5*(eta_LOS[0]*pres[0] + eta_LOS[-1]*pres[-2] + sum(eta_LOS[1:-1]*(pres[0:-2]+pres[1:-1])))
        
        return 0.5*(sum(pres*(eta_LOS[0:-1]+eta_LOS[1:])))    
        
           
    def plot_both_planes(self, cmap="hot", levels=100, vmin=-8, vmax=-4, save=False, savetag="", figfile=None):
        ...
        figfile - full name of the output file if specified. Otherwise uses a default. 
        ...
        if save: 
            if figfile is None:
                ...
                print ('Saved to: ', figfile)
                fig.savefig(figfile) 


    def plot_integrated_panels(self, cmap="inferno", levels=100, vmin=0, vmax=100, save=False, savetag="", figfile=None):
        '''This will plot an integration along either the z-axis or the y-axis instead of the central planes. It will plot on a linear scale. ''' 
        
        shape = self.eta_3d.shape 
        
        #Do integration along y-axis first to the equivalent of the XZ plane plot above. 
        #Do each direction separately. 
        self.int_eta_y = np.zeros((shape[0], shape[2]))
        for i in range(shape[0]):
            for j in range(shape[2]): 
                ...
                #Get resolution along the y axis. 
                yres = y[1:] - y[0:-1]
                
                #Integrate along this axis. 
                self.int_eta_y[i,j] = self.trapezium_rule_varied(yres, eta)
                
        #Do integration along z-axis first to the equivalent of the XY plane plot above. 
        #Do each direction separately. 
        self.int_eta_z = np.zeros((shape[1], shape[2]))
        for i in range(shape[1]):
            for j in range(shape[2]): 
                ...
                #Integrate along this axis. 
                self.int_eta_z[i,j] = self.trapezium_rule_varied(zres, eta)
           
        #Get associated XZ or XY arrays. 
        self.x_y, self.z_y = np.meshgrid(self.x, self.z) 
        self.x_z, self.y_z = np.meshgrid(self.x, self.y) 
        
        #Convert integrated emissivity from units with RE to just cm and eV to keV. 
        self.int_eta_y = self.int_eta_y*637100
        self.int_eta_z = self.int_eta_z*637100
        ...
        # Constant y. 
        cont1 = ax1.contourf(self.x_y, self.z_y, self.int_eta_y, cmap=cmap, levels=levels, vmin=vmin, vmax=vmax)
        ax1.set_xlabel('X [RE]')
        ax1.set_ylabel('Z [RE]')
        ax1.set_title("XZ Plane")
        ax1.set_aspect("equal")
        ...
        # Constant y. 
        cont2 = ax2.contourf(self.x_z, self.y_z, self.int_eta_z, cmap=cmap, levels=levels, vmin=vmin, vmax=vmax)
        ax2.set_xlabel('X [RE]')
        ax2.set_ylabel('Y [RE]')
        ax2.set_title("XY Plane")
        ax2.set_aspect("equal")
        ...
        # Add colourbar. 
        cbar = plt.colorbar(cont2, ax=ax2, shrink=0.5)
        cbar.set_label("Integrated Emissivity\n"+r"[keV cm$^{-2}$ s$^{-1}$]")
        level_min = int(np.ceil(cont2.levels.min()))
        level_max = int(np.floor(cont2.levels.max()))
